...
#     # VALIDATION ERRORS (Handle in Serializer)
#     def validate_category_name(self, value):
...
#             if self.instance:
#                 if not Category.objects.filter(category_name__iexact=value).exclude(id=self.instance.id).exists():
#                     return value
#             raise serializers.ValidationError("A category with this name already exists.")
        
#         return value.strip().title()  # Clean and format the name
...
#         # Example: Business categories can't have certain names
#         if data.get('type') == 'business' and 'personal' in data.get('category_name', '').lower():
#             raise serializers.ValidationError("Business categories cannot contain 'personal' in the name.")
#         return data
